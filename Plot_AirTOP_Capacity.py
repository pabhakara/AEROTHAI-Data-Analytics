...
def plot_workload_vs_entry_per_sector(df_workload_rolling, df_entry_rolling, save_plots=False, save_path="sector_plots"):
    """
    Creates scatter plots for each sector with sector entry count on X-axis
    and workload on Y-axis, including:
    - Linear regression trendline (Black)
    - Red horizontal line at workload capacity (70%) (Always Plotted)
    - Green vertical line where regression crosses 70% workload (Sector Capacity, if applicable)
    - R² score to indicate regression fit quality
    - Capacity value label on the X-axis
    - Option to save plots as PNG and PDF

    Args:
        df_workload_rolling (DataFrame): DataFrame with rolling workload per sector.
        df_entry_rolling (DataFrame): DataFrame with rolling sector entry count.
        save_plots (bool): If True, saves plots as PNG and PDF.
    ...
    df_scatter = pd.merge(df_workload_rolling, df_entry_rolling, on=["Time", "Sector"], how="inner")

    # Get unique sectors
    sectors = df_scatter["Sector"].unique()

    for sector in sectors:
        df_sector = df_scatter[df_scatter["Sector"] == sector]

        # Extract X (entry count) and Y (workload) data
        x_data = df_sector["Count"].values.reshape(-1, 1)  # Reshape for sklearn
        y_data = df_sector["Workload"].values

        # Ensure sufficient data for regression
        if len(x_data) < 2 or np.var(x_data) == 0:
            print(f"Skipping sector {sector} due to insufficient or constant data.")
            continue
        ...
        # Calculate Sector Capacity (where workload = 70%)
        sector_capacity = (70 - intercept) / slope if slope != 0 else None

        # Determine y-axis limits
        min_y = min(y_data.min(), 65)  # Ensure lower bound is at least 65 for visibility
        max_y = max(y_data.max(), 75)  # Ensure upper bound is at least 75 to include 70% line

        # Create scatter plot
        fig = go.Figure()

        # Scatter plot (Sector Entry Count vs Workload)
        ...
        # Workload Capacity Line (Red, 70%) - Always Plotted
        fig.add_trace(go.Scatter(
            x=[x_data.min(), x_data.max()],  # From min to max entry count
            y=[70, 70],  # Constant workload threshold
            mode='lines',
            name='Workload Capacity (70%)',
            line=dict(color='red', dash='dot')
        ))

        # Vertical Line at Sector Capacity (Green, only if applicable)
        if sector_capacity is not None and x_data.min() <= sector_capacity <= x_data.max():
            fig.add_trace(go.Scatter(
                x=[sector_capacity, sector_capacity],
                y=[min_y, 70],  # From bottom to workload limit
                mode='lines',
                name=f'Sector Capacity: {sector_capacity:.1f} entries',
                line=dict(color='green', dash='dot')
            ))

            # Annotate sector capacity value on the X-axis
            fig.add_annotation(
                x=sector_capacity,
                y=min_y,  # Placed on X-axis
                text=f"{sector_capacity:.1f} entries",
                showarrow=True,
                arrowhead=2,
                ax=0,
                ay=40,  # Move label slightly below the axis
                font=dict(color="black", size=12)
            )

        # Layout settings
        fig.update_layout(
            title=f"Sector: {sector} - Workload vs Entry Count",
            xaxis=dict(title="Sector Entry Count (1-hour rolling)", range=[x_data.min() - 1, x_data.max() + 1]),
            yaxis=dict(title="Workload (%)", range=[min_y - 5, max_y + 5]),  # Ensure 70% is always in the range
        ...
        if save_plots:
            png_path = os.path.join(save_path, f"{sector}_scatter.png")
            pdf_path = os.path.join(save_path, f"{sector}_scatter.pdf")
...
scenario = "/BKK_FIR_AIRAC-2024-13_BKK_RWY_19-20-21_130_percent"
...
plot_workload_vs_entry_per_sector(df_workload_rolling, df_entry_rolling)
# Save results
# df_entry_rolling.to_csv("/Users/pongabha/Desktop/df_sector_entry_count.csv")
# df_exit_rolling.to_csv("/Users/pongabha/Desktop/df_sector_exit_count.csv")
