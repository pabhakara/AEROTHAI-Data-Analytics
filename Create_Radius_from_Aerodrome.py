...
with conn_postgres_target:
    ...
    postgres_sql_text = f" DROP TABLE IF EXISTS airac_current_vt.airports_buffer; \n" \
                        f" SELECT * "
    ...
    postgres_sql_text += f" INTO airac_current_vt.airports_buffer \n"\
                         f" FROM airac_current_vt.airports; \n"
    cursor_postgres_target.execute(postgres_sql_text)
    conn_postgres_target.commit()