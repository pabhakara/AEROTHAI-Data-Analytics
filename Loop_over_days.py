...
with conn_postgres_source:
    ...
    for date in date_list[:-1]:
        ...
        postgres_sql_text = f"SELECT x.dof,x.acid,x.actype,x.dep,x.dest,x.star,MIN(EXTRACT(EPOCH FROM ttg))  as ttg "\
                f"FROM "\
                f"(SELECT t.dof,t.acid,t.actype,t.dep,t.dest, " \
                f"        s.waypoint_identifier as star, s.time_of_track as time_at_star,t.aldt,t.aldt - s.time_of_track as ttg "\
                f"FROM "\
                f"	(SELECT t.*,b.waypoint_identifier,b.airport_identifier "\
                ...
                f"	track.track_cat62_{yyyymmdd} t "\
                ...
                f"GROUP BY x.dof,x.acid,x.actype,x.dep,x.dest,x.star "\
                f"UNION "
