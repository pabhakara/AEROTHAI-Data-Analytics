...
AND (departure LIKE 'OMDB%' OR arrival LIKE 'OMDB%')
"""
from sqlalchemy import text
gdf = gpd.read_postgis(text(sql), engine, geom_col='geom')
...
for _, row in grouped.iterrows():
    ...
    for i, line in enumerate(lines):
        ...
        traces.append(go.Scattergeo(
            ...
            opacity=0.1,
            name=f"{route} ({count} flights)" if i == 0 else None,
            showlegend=(i == 0),
            text=hover_text,
            hoverinfo='text'
        ))
...
fig.write_html(f"{root_directory}omdb_routes_globe.html")
