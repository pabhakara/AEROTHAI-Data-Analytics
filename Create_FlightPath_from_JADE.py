...
with conn_postgres:
    ...
    try:
        ...
        if conn_mysql.is_connected():
            ...
            offset_lat_2 = offset_2 * 1.852 / 60. * math.sin(
                math.radians((float(temp_2['latitude'])) - center_map_latitude)) * lat_offset_scale
            ...
            offset_lon_1 = offset_3 * 1.852 / 60. * math.sin(
                math.radians((float(temp_1['longitude'])) - center_map_longitude)) * lon_offset_scale
            ...
            while k < num_of_records - 1:
                while (temp_1['flight_id'] == temp_2['flight_id']) and \
                    ...
                    offset_1 = (Re ** 2 + (
                                (center_map_latitude - float(temp_1['latitude'])) * 60 * 1.852) ** 2) ** 0.5 - Re
                    offset_lat_1 = offset_1 * 1.852 / 60. * math.sin(
                        math.radians((float(temp_1['latitude'])) - center_map_latitude)) \
                                   * lat_offset_scale
                    latitude_1 = str(float(temp_1['latitude']) + offset_lat_1)

                    offset_3 = (Re ** 2 + (
                                (center_map_longitude - float(temp_1['longitude'])) * 60 * 1.852) ** 2) ** 0.5 - Re
                    offset_lon_1 = offset_3 * 1.852 / 60. * math.sin(
                        math.radians((float(temp_1['longitude'])) - center_map_longitude)) \
                                   * lon_offset_scale
                    longitude_1 = str(float(temp_1['longitude']) + offset_lon_1)
                ...
                offset_2 = (Re ** 2 + ((center_map_latitude - float(temp_2['latitude'])) * 60 * 1.852) ** 2) ** 0.5 - Re
                offset_lat_2 = offset_2 * 1.852 / 60. * math.sin(
                    math.radians((float(temp_2['latitude'])) - center_map_latitude)) \
                               * lat_offset_scale
                latitude_2 = str(float(temp_2['latitude']) + offset_lat_2)
                ...
                offset_4 = (Re ** 2 + (
                            (center_map_longitude - float(temp_2['longitude'])) * 60 * 1.852) ** 2) ** 0.5 - Re
                offset_lon_2 = offset_4 * 1.852 / 60. * math.sin(
                    math.radians((float(temp_2['longitude'])) - center_map_longitude)) \
                               * lon_offset_scale
                longitude_2 = str(float(temp_1['longitude']) + offset_lon_2)
