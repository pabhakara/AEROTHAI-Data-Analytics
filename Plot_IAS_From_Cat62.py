...
with conn_postgres_source:
    ...
    for STAR in STAR_list:
        postgres_sql_text = f"SELECT DISTINCT t.flight_key, f.actype, '{STAR}' " \
                            ...
                            f"WHERE (f.dep LIKE '%' AND f.dest LIKE 'VTBS') " \
                            f"AND t.measured_fl > 1 " \
                            f"AND f.flight_key LIKE 'BKP%' " \
                            f"AND f.frule LIKE '%'; "
...
with conn_postgres_source:
    ...
    for flight_key in flight_key_list:
        ...
        postgres_sql_text = f"SELECT t.flight_key, t.app_time, t.sector, t.dist_from_last_position, t.measured_fl " \
                            f", t.ias_dap, t.final_state_selected_alt_dap " \
        ...
        df = pd.DataFrame(record, columns=['flight_key','app_time','sector','dist_from_last_position','measured_fl',
                                           'ias_dap','final_state_selected_alt_dap','latitude','longitude'])
        ...
        df['distance_from_THR'] = df['accumulated_distance'].max()-df['accumulated_distance']
        ...
        # Add figure title
        fig.update_layout(
            title_text=f"IAS of Flight {flight_key}"
            #title_text=f"Vertical Profile"
        )

        # Set x-axis title
        fig.update_xaxes(title_text="Distance from THR (NM)")
